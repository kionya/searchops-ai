  ...
  return (
    ...
    >
      <section aria-labelledby="observability-heading">
        ...
        <section aria-label="운영 지표 export" className="searchops-table-section">
          ...
          <div className="searchops-table-scroll">
            <table className="searchops-table" style={{ minWidth: 760 }}>
              <thead>
                <tr>
                  <th>지표</th>
                  <th>값</th>
              ...
              <tbody>
                {Object.entries(dashboard.metrics.requests.byStatus).map(([status, count]) => (
                  <tr key={status}>
                    <td className="searchops-code">http.status.{status}</td>
                    <td>{count}</td>
                    <td>API</td>
                  </tr>
                ))}
        ...
        <section aria-label="운영 알림" className="searchops-table-section">
          <header className="searchops-table-head">
            <div>
              ...
              <tbody>
                {dashboard.metrics.alerts.length === 0 ? (
                  ...
                  dashboard.metrics.alerts.map((alert) => (
                    <tr key={alert.id}>
                      <td className="searchops-code">{alert.id}</td>
                      <td>
                        <AlertTonePill
                      ...
                      <td>{formatOperationalSource(alert.source)}</td>
                      <td>{alert.message}</td>
                    </tr>
                  ))
                )}
