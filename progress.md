...
**전환 중 겪은 사고: turbo 캐시가 Prisma 엔진을 지웠다.** `packages/db/src/generated/prisma` 는 gitignore 대상인데 turbo `build.outputs` 에 없었다. `packages/db` 가 캐시 히트하면 `dist/**` 만 복원되고 엔진이 사라지는데, **소스가 안 바뀐 커밋에서만** 그렇게 되므로 간헐적으로만 재현된다(web 만 캐시 미스인 커밋에서 터졌다). 게다가 복사 스크립트가 엔진이 없을 때 `exit 0` 이라 **엔진 없는 람다가 초록불로 배포**됐고, 로그인한 사용자만 500 을 봤다. `outputs` 에 넣어 원인을 막고, 복사 스크립트가 빌드를 세우게 해 같은 부류가 다시 조용히 나가지 않게 했다.

⚠️ 이 과정에서 예전 판단 하나가 틀린 것으로 드러났다: **"pg 드라이버 어댑터를 쓰면 엔진 바이너리가 필요 없다" 는 사실이 아니다.** 어댑터는 Rust 쿼리 엔진의 접속 계층만 바꾼다 — 쿼리 컴파일은 그대로 엔진이 한다. 그 말을 믿고 `next.config.mjs` 의 트레이싱 설정을 지웠다면 배포가 통째로 죽었다. 관련 주석을 전부 바로잡았다.

**`/api/deployment` 에 `writable` 을 추가했다.** `reachable` 은 `select 1` 이라 예전 읽기 전용 역할로도 `true` 가 나와, 역할을 갈아끼웠는지 밖에서 확인할 수 없었다. 확인 없이 예전 역할을 지우면 대시보드가 통째로 죽는다. 역할명이 아니라 불리언만 낸다 — DB 사용자명은 자격증명의 절반이다.
...
#### 커넥터를 살리려고 API 를 되살린다 (2026-08-17, 운영자 작업 대기)

커넥터 화면에서 계정 연결이 비활성이고 리소스 저장이 안 된다는 보고가 들어왔다. 원인은 하나 — **커넥터의 읽기·쓰기가 전부 `provider-accounts.ts` 의 `request()` 를 지나고, API 주소가 없으면 거기서 `not_configured` 로 던진다.** 리소스 입력칸만 잠기지 않아 "타이핑은 되는데 저장이 안 되는" 상태였다(고쳤다).

**"Vercel 에 암호화 키를 두지 않는다" 는 "API 를 만들지 않는다" 가 아니었다.** 키를 가진 프로세스를 웹 밖에 두면 규칙을 지키면서 커넥터를 살릴 수 있다. 그게 원래 설계다. 그래서 API 를 별도 호스트에 되살리기로 했다.

준비해 둔 것:

- **`apps/api/Dockerfile`** — 호스트 비종속. CI 의 `api-image` 잡이 매 푸시마다 빌드와 부팅을 확인한다. Dockerfile 은 배포 직전에야 쓰는 물건이라, 여기서 안 돌리면 가장 급할 때 처음 깨진다.
- **`apps/worker/src/batch-connector-sync.ts`** — 배치가 커넥터 동기화를 직접 실행한다. API 의 "지금 동기화" 는 큐에 넣기만 하고, 큐를 소비하는 워커를 상시 띄우지 않으면 영원히 대기하기 때문이다. **워커 없이도 매일 GSC/GA4 데이터가 들어온다.**
- **`docs/API_DEPLOYMENT.md`** — env 목록, OAuth 리디렉트 URI, GitHub Secret, 검증 순서, 증상별 처방.

⚠️ 남은 것은 운영자 작업이다: 컨테이너 호스트 1개 + Redis 1개 + Google OAuth 클라이언트. 호스트별 무료 한도는 자주 바뀌므로 현재 요금을 직접 확인해야 한다(⚠️ 검증필요).

⚠️ 대시보드는 API 가 살아나도 **DB 직접 경로를 계속 쓴다.** `SEARCHOPS_WEB_DATABASE_URL` 을 지우지 마라 — 더 빠르고, API 가 죽어도 대시보드는 산다.
...
- T6 (2026-09-21): `packages/reports`(입력은 결과 JSON 만, DB 접근 없음) — `renderDiagnosisHtml`(A~J)·`renderProposalHtml`(1~11), 절마다 `data-source`(리포트 id·run·liveShare), 경쟁사 절 `data-audience="internal"`(external 빌드는 제거), liveShare<1 경고 박스, 목표 수치 공란이면 Zod 가 던져 빌드 실패. `GET /sites/:id/reports/{diagnosis|proposal}?run=&target*=&audience=` → HTML. ⚠️ 검증필요: 절 순서는 harness-suite 프레임워크 §2·§3 정본과 대조(다른 프로젝트라 승인 전 미확인).
