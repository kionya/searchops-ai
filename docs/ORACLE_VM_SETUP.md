...
**콘솔이 권하는 "다른 가용 도메인을 시도하라"는 도쿄·오사카에서는 따를 수 없다.**
두 리전 모두 AD가 1개뿐이라 고를 대상이 없다. 실제로 남은 자유도는 두 가지다.

1. **fault domain을 지정하지 않는다.** AD 하나 안에 FD는 3개 있고, 지정하지 않으면 OCI가
   가용한 FD를 고른다(Oracle 공식 권고). 콘솔에서는 Advanced options → Placement를 비워둔다.
2. **요청 크기를 줄인다.** 용량 판정은 요청 크기의 함수라 1 OCPU/6GB가 2 OCPU/12GB보다 들어갈 자리를 찾기 쉽다.
   이 워크로드는 1 OCPU/6GB로도 충분하다(컨테이너 3개 합쳐 1GB 미만, 빌드도 스왑 없이 통과한다).
   오히려 유휴 회수 정책상 사용률이 올라가 유리하다.
   ⚠️ 작게 확보한 뒤 키우는 리사이즈는 신규 생성과 동일하게 취급되어 다시 실패할 수 있으니 기대하지 않는다.
   부트 볼륨 크기는 컴퓨트 호스트 용량과 무관하므로 줄여도 도움이 되지 않는다.

수동 재시도 대신 `scripts/dev/oci-a1-retry.sh`를 쓴다. 2분 간격으로 큰 구성과 작은 구성을
번갈아 시도하고, 용량 에러만 재시도하며 `LimitExceeded` 같은 것은 즉시 중단한다.
초 단위로 두드리면 OCI가 429로 막으므로 간격을 줄이지 않는다.

PAYG(유료) 업그레이드가 확보에 유리하다는 보고가 많지만 ⚠️ Oracle 공식 근거는 없고 실패 사례도 있다.
업그레이드해도 무료 한도(2 OCPU/12GB) 안이면 과금되지 않으나, 넘기면 월 $28 수준이 붙으므로
전환 전에 Budgets 알림을 먼저 설정한다.
