...
`POST /sites/:siteId/cms/webhooks/:cmsType` accepts provider-specific CMS webhook payloads for `wordpress`, `webflow`, and `headless`. `packages/connectors` normalizes the provider payload into the same `CmsContentUpdatedEventRequest`, then the API applies signature verification, site URL scoping, compliance recheck, and response contract. WordPress and Webflow provider routes can use selected native signature headers as a fallback to the SearchOps HMAC contract:

- WordPress: `x-wp-webhook-timestamp` and `x-wp-webhook-signature`.
- Webflow: `x-webflow-timestamp` and `x-webflow-signature`.

Native signatures are HMAC-SHA256 over the timestamp plus canonical provider payload and are checked against the same provider-scoped CMS webhook secret map and replay window. The route still does not fetch from or publish to a live CMS.
