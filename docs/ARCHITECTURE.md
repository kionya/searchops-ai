...
Railway API and Worker receive the same storage mode and active/previous keyring. Vercel Web never receives database, Redis, OAuth client secret/state secret, provider API, customer, or credential-encryption secrets. Its currently required service-auth and alert/log-drain sink tokens remain non-`NEXT_PUBLIC` server-only values and must never enter client code. The API writes encrypted organization accounts; the Worker reads/decrypts them per job. Queue payloads, API responses, logs, sync summaries, and dead-letter records contain no raw or encrypted credential fields.
...
`apps/api` owns the HTTP boundary for schema recommendation creation/history, work order conversion, snapshot-based recheck, crawl orchestration for recommendation rechecks, and rich-result validation enqueue. It may call deterministic `packages/schema-core`, validate requests and responses through `packages/types`, scope recommendation page URLs to the registered site domain/subdomains, persist recommendation drafts through repository ports, and enqueue worker jobs with the `schemaRecommendationId` needed for handoff. It must not call LLM providers, live rich-result validators, or CMS publish adapters for Phase 8 schema recommendation flows.

`apps/worker` owns the queued schema recheck handoff after a crawl finishes and rich-result validation job processing. When a crawl job carries `schemaRecommendationId`, the worker persists UrlRecords, extracts observed JSON-LD types from the matching crawler snapshot through `packages/schema-core`, and asks `packages/db` to update recommendation evidence/status plus the linked work order status. For rich-result validation jobs, the worker runs deterministic offline validation by default or an explicitly injected connector validator and stores the result as recommendation evidence. The worker does not publish JSON-LD or call live rich-result tooling unless a deployment injects that connector boundary.
...
`apps/api` owns the HTTP boundary for GEO visibility report creation/history, monitor job enqueue, and report-to-work-order conversion. It may call deterministic `packages/geo-core` and `packages/workorders`, validate requests and responses through `packages/types`, scope target domains to the registered site domain or subdomain, and persist through repository ports. It must not call LLM providers or live AI answer providers for Phase 9 flows.

`apps/worker` owns the GEO answer monitor runtime handoff. The worker consumes typed `geo-answer-monitor` jobs, calls a monitor batch function through an injected connector boundary, evaluates the returned observations through deterministic `packages/geo-core`, and persists `GeoVisibilityReport` history through `packages/db`. Fixture monitor adapters remain the default; live provider clients are explicit injection only.

`packages/db` owns GEO visibility persistence. `GeoVisibilityReport` stores report history for dashboard use, including observations and citations as JSON evidence. Re-running a visibility evaluation or monitor job creates another history row so the dashboard can show trendable snapshots. `WorkOrder.geoVisibilityReportId` links one report to at most one idempotent task.
...
Production hardening starts at runtime boundaries before deeper platform policy work. `apps/api` owns HTTP rate limiting and request metrics because those controls depend on request identity and deployment topology. The default limiter is in-memory and deterministic for local/test environments, while the API runtime wires a Redis-backed store through `ioredis` when rate limiting is enabled in deployment. Multi-instance deployments can use the same route-handler boundary without changing API code.
...
`apps/api` owns the operations boundary for dead-letter inspection. It can list and clear dead-letter queue entries through BullMQ-backed adapters, while local tests use an in-memory store. Replay uses queue-specific schemas plus deterministic replay job IDs, requires an operator-supplied source-of-truth payload, and can remove the dead-letter entry only after enqueue succeeds.
...
Operations hardening plans are exposed through the API as deterministic plan builders. Backup restore drills, migration deployment gates, secret rotations, and dead-letter replay plans return typed steps and statuses without touching live databases or exposing raw secret values. Dedicated execution routes dispatch restore drills and secret rotations to configured HTTP deployment executors, while dead-letter replay remains queue-specific and idempotent by replay job ID. `apps/web` links these plans through the operations hub, using API data when configured and deterministic fixture data otherwise.

## Productization Boundary

Productization readiness is tracked as an operations report instead of direct provider calls. `apps/api` exposes `/ops/productization`, which summarizes Auth/RBAC provisioning, tenant isolation evidence, invite policy, billing policy, production domain, legal docs, and onboarding state from env and deterministic metadata only. It does not call Auth0, Clerk, Supabase Auth, Stripe, DNS providers, email providers, or legal services.

`apps/web` exposes `/ops/productization` and `/onboarding` as operator-facing surfaces. They can render from API data or deterministic fixtures and keep live billing, live IdP invites, and customer-specific onboarding completion as deployment/product follow-up until provider choices are made.
